...
const cargarTestAnforaTema3 = async () => {
  try {
    ...
    const tema3 = curso.temas?.find(t => t.numeroTema === 3);
    if (!tema3) {
      console.error('✗ ERROR: No se encontró el Tema 3 en el curso.');
      console.error('   Temas disponibles:', curso.temas?.map(t => `Tema ${t.numeroTema}: ${t.titulo}`));
      process.exit(1);
    }
    console.log(`✓ Tema 3 encontrado: "${tema3.titulo}" (ID: ${tema3._id})`);

    const testData = JSON.parse(
      readFileSync(join(__dirname, '../data/test-anfora-tema3.json'), 'utf-8')
    );

    testData.cursoId = curso._id.toString();
    testData.temaId  = tema3._id.toString();
