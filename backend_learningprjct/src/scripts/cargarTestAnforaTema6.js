...
const cargarTestAnforaTema6 = async () => {
  try {
    ...
    const tema6 = curso.temas?.find(t => t.numeroTema === 6);
    if (!tema6) {
      console.error('✗ ERROR: No se encontró el Tema 6 en el curso.');
      console.error('   Temas disponibles:', curso.temas?.map(t => `Tema ${t.numeroTema}: ${t.titulo}`));
      process.exit(1);
    }
    console.log(`✓ Tema 6 encontrado: "${tema6.titulo}" (ID: ${tema6._id})`);

    const testData = JSON.parse(
      readFileSync(join(__dirname, '../data/test-anfora-tema6.json'), 'utf-8')
    );

    testData.cursoId = curso._id.toString();
    testData.temaId  = tema6._id.toString();
