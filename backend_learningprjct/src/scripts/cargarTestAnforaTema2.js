...
const cargarTestAnforaTema2 = async () => {
  try {
    ...
    // Obtener el Tema 2 (numeroTema === 2)
    const tema2 = curso.temas?.find(t => t.numeroTema === 2);
    if (!tema2) {
      console.error('✗ ERROR: No se encontró el Tema 2 en el curso.');
      console.error('   Temas disponibles:', curso.temas?.map(t => `Tema ${t.numeroTema}: ${t.titulo}`));
      process.exit(1);
    }
    console.log(`✓ Tema 2 encontrado: "${tema2.titulo}" (ID: ${tema2._id})`);

    // Leer el JSON del test
    const testData = JSON.parse(
      readFileSync(join(__dirname, '../data/test-anfora-tema2.json'), 'utf-8')
    );

    // Inyectar los IDs reales
    testData.cursoId = curso._id.toString();
    testData.temaId  = tema2._id.toString();
